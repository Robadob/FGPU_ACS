...
{	
	...
	glUniform1i(pvs_active_exit, 0);

	//draw lod 0 geometry
	for (i=0; i<getPedestrianCount(); i++)
	{

		GL_CALL(glVertexAttrib1f(pvs_instance_index, (float)count));
		count++;
		
		GL_CALL(glBindBuffer(GL_ARRAY_BUFFER, lod1l_verts_vbo));
		GL_CALL(glVertexPointer(3, GL_FLOAT, 0, 0));

		GL_CALL(glEnableVertexAttribArray(pvs_position_r));
		GL_CALL(glBindBuffer(GL_ARRAY_BUFFER, lod1r_verts_vbo));
		GL_CALL(glVertexAttribPointer(pvs_position_r, 3, GL_FLOAT, 0, 0, 0));

		GL_CALL(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lod1_elem_vbo));

		GL_CALL(glEnableClientState(GL_VERTEX_ARRAY));

		GL_CALL(glDrawElements(GL_TRIANGLES, lod1_f_count*3, GL_UNSIGNED_INT, 0));

		GL_CALL(glDisableClientState(GL_VERTEX_ARRAY));
		GL_CALL(glDisableVertexAttribArray(lod1r_verts_vbo));
	}
