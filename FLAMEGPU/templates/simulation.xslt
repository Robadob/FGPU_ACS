...
<xsl:if test="gpu:partitioningSpatial">/* Spatial Partitioning Variables*/
  ...
  const unsigned long binCountBits_<xsl:value-of select="xmml:name"/> = (unsigned long)ceil(log(((<xsl:value-of select="gpu:partitioningSpatial/gpu:xmax"/>-<xsl:value-of select="gpu:partitioningSpatial/gpu:xmin"/>) * (<xsl:value-of select="gpu:partitioningSpatial/gpu:ymax"/>-<xsl:value-of select="gpu:partitioningSpatial/gpu:ymin"/>) * (<xsl:value-of select="gpu:partitioningSpatial/gpu:zmax"/>-<xsl:value-of select="gpu:partitioningSpatial/gpu:zmin"/>)) / (float)(<xsl:value-of select="gpu:partitioningSpatial/gpu:radius"/>*<xsl:value-of select="gpu:partitioningSpatial/gpu:radius"/>*<xsl:value-of select="gpu:partitioningSpatial/gpu:radius"/>)) / log(2));
  #endif
...
void <xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(cudaStream_t &amp;stream){
  ...
  <xsl:if test="gpu:partitioningNone or gpu:partitioningSpatial or gpu:partitioningGraphEdge">
    ...
    {
    unsigned int *_t = d_xmachine_message_pedestrian_location_keys;
    d_xmachine_message_pedestrian_location_keys = d_xmachine_message_pedestrian_location_keys_swap;
    d_xmachine_message_pedestrian_location_keys_swap = _t;
    }
    {
    unsigned int *_t = d_xmachine_message_pedestrian_location_values;
    d_xmachine_message_pedestrian_location_values = d_xmachine_message_pedestrian_location_values_swap;
    d_xmachine_message_pedestrian_location_values_swap = _t;
    }
    gpuErrchkLaunch();
